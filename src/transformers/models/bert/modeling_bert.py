...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # Added avg argument (Arnout).
        first_token_tensor = torch.mean(hidden_states, 1) if self.config.avg_pool else hidden_states[:, 0]
    ...
    def prune_all_mlp_neurons_except(self, neurons: Dict[int, Tuple[int]]) -> None:
        for i, l in enumerate(self.encoder.layer):
            if i in neurons.keys():
                ...
                l.intermediate.dense.bias = nn.Parameter(l.intermediate.dense.bias.index_select(0, indices), requires_grad=True)
                # output
                l.output.dense.weight = nn.Parameter(l.output.dense.weight.index_select(1, indices).reshape(self.config.hidden_size, dim), requires_grad=True)
